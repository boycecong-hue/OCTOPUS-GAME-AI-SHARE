# 为什么是现在：AI Coding 正在从辅助走向闭环

这一页只回答一个问题：为什么 2026 年值得专门讲 AI Coding。

## 1. 我们正在经历的不是工具升级，而是工作方式迁移

过去几年，AI 编程经历了非常清晰的能力跃迁。

```mermaid
timeline
    title AI Coding 能力跃迁
    2022 - 2023 : 智能补全 : 提高打字速度 : 帮你写下一段
    2023 - 2024 : 对话式编辑 : 通过自然语言改局部代码 : 帮你完成一个子任务
    2025 - 2026 : 自主 Agent : 读代码、写代码、跑测试、修问题 : 帮你完成一整件事
```

### 这次变化真正改变了什么
- **交互对象变了**：从“补一段代码”变成“完成一个目标”。
- **反馈链路变了**：AI 不再只输出文本，而是开始依据测试、日志、命令结果继续行动。
- **开发者角色变了**：人从手工实现者，逐步转向目标定义者与结果验收者。

## 2. 这场分享会要解决什么问题

今天不想把内容做成工具说明书，而是回答三件事：
- **第一**：AI Coding 到底变到了哪个阶段。
- **第二**：为什么 Agent 和 CLI 会成为主战场。
- **第三**：团队怎样把这种能力真正放进研发流程里，而不是停留在 demo。

## 3. 接下来的讲述路径

这套分享会按下面的顺序推进：
1. **先看模型层**：国内外大模型的性能、费用和使用边界怎么判断。
2. **再讲工具形态**：为什么是 CLI / Agent，而不是普通补全。
3. **再讲最小术语**：把后面会反复出现的概念对齐。
4. **再讲研发闭环**：需求如何变成计划、实现、验证和交付。
5. **最后讲治理与角色变化**：企业怎么落地，开发者怎么进化。

---

> 这场分享的重点不是“AI 会不会写代码”，而是“AI 是否已经开始参与真实交付”。