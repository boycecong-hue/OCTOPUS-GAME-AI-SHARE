...
2. **再看产品与执行体系**：看 Claude Code、Codex、Gemini 分别怎样组织执行体系。
3. **再拆调用链路与执行机制**：把 Agent、Harness、Skills、Hooks、MCP、LSP 放进同一张运行时图里。
4. **再补一个实操控制台**：CC Switch 这类个人 AI 工作台统一控制台，为什么会在长期使用阶段变重要。
